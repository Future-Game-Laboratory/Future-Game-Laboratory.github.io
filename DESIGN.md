...
界面借鉴 Steins;Gate 官网的排版张力与遮罩显影，但不复制其人物、标志、文案或纹理。首页以社团作品海报的顺序轮播作为主视觉，栏目骨架参考晓 Records 的「品牌导航—主视觉—News 时间流」关系；公开邮箱与 SNS 位于首页右下角，Contact 只负责表单。Edits 是主导航之外的工具入口。不设置 Discography、Music 或独立 SNS 内容分区。
...
- 首页只保留作品海报轮播、日期驱动的 News 动态流和右下角联系方式，不再设置左右分栏、四格目录、栏目侧栏或底部宣言。
...
- **Menu Toggle：** 移动端右上角使用 `2.75rem`、4px 圆角的正方形按钮；三条细线在约 260ms 内无弹跳地切换为叉号，关闭时沿相同路径反向还原，减弱动效模式立即切换。
- **Poster Cycle：** 海报以约 5.2 秒间隔按文件名顺序循环，使用克制的透明度转场；悬停或键盘聚焦时暂停。
- **Nixie Boot：** 纯白环境中的八枚细长冷阴极辉光管以 `XX.XXXXXX` 显示加载百分比；玻璃罩、冷色阳极网、电极残影、浅色陶瓷管座和琥珀阴极辉光共同建立真实感。加载层只显示管体，计数约 1.65 秒，同一会话只出现一次。

所有运动都必须可中断，不阻塞页面内容，并完整支持 `prefers-reduced-motion`。

## 6. Components

- **Navigation：** 不显示 Home 项；点击页宽左侧的社团标识返回首页，桌面导航靠同一页宽的右上角显示，其他栏目只显示全大写英文名称。移动端右上角以正方形汉堡按钮打开目录，按钮在相同位置切换为叉号；目录使用 `#090909` 全屏背景、白色品牌标识与白色大标题。Edits 仅出现在移动端目录页脚，站点页脚不再重复任何导航入口。
...
- **Homepage Contact：** 首页右下角明文显示邮箱；小红书、X、哔哩哔哩按非空配置渲染圆形图标按钮，全部为空时不渲染整块内容。
