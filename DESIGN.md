...
界面借鉴 Steins;Gate 官网的排版张力与遮罩显影，但不复制其人物、标志、文案或纹理。首页以社团作品海报的顺序轮播作为主视觉，栏目骨架参考晓 Records 的「品牌导航—主视觉—News 时间流」关系；公开邮箱与 SNS 位于首页右下角，Contact 只负责表单。Edits 仅通过直接访问 `/edits/` 使用，不在公开页面提供入口。不设置 Discography、Music 或独立 SNS 内容分区。
...
- **Nixie Boot：** 纯白环境中的八枚细长冷阴极辉光管以 `XX.XXXXXX` 显示加载百分比；白热阴极核心、琥珀扩散光、玻璃高光、冷色阳极网、暗置电极残影和陶瓷底座反光共同建立真实感。加载层只显示管体，计数约 1.65 秒，同一会话只出现一次。
...
- **Nixie Loader：** 背景固定为 `#FFFFFF`，八枚独立细长管体组成两位整数和六位小数；数字使用白热核心、近场橙光和远场琥珀光三层发光，玻璃与底座只反射这些真实光源。除管体和小数点外不显示标题、状态、轨道或跳过按钮。
