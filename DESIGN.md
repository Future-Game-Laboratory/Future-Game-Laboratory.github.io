...
- **News Feed：** 首页最多显示 5 条；标题与摘要保持紧凑字号，日期位于条目右上角，右下角使用带悬停位移动效的三角形目标标记；内容增长后仍保持按时间扫描。
- **About Document：** 内容来自独立 Markdown，使用简单标题、段落、图片与链接。
- **Homepage SNS：** 仅使用统一尺寸的白底黑色图标按钮，不显示可见标题或文字标签；组织 GitHub 固定显示并排在 RSS 之前，RSS 始终显示，小红书、X、哔哩哔哩仅在链接非空时逐项渲染。邮箱不混入首页 SNS，仍作为 Contact 表单的收件配置。
...
- **Edits Admin：** `/edits/` 是独立的产品界面：深色固定侧栏、白色顶部账户与仓库权限区、内容列表—编辑工作区、标准表单与明确的保存状态。桌面端可收起侧栏，移动端使用抽屉导航和单列编辑器；不继承公开站的大标题揭示、装饰性故障动效或启动动画。
